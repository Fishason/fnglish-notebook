...
export class TencentTranslationService {
  ...
  async translateText(text: string, from = 'en', to = 'zh'): Promise<TranslationResult> {
    try {
      const params = {
        SourceText: text,
        Source: from,
  ...
  async translateImage(imageBase64: string, from = 'auto', to = 'zh'): Promise<TranslationResult> {
    try {
      const params = {
