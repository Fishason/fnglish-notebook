...
export class NotionService {
  ...
  private titlePropertyName: string = '';
  ...
  async validateDatabase(): Promise<string> {
    try {
      const response = await this.client.databases.retrieve({ database_id: this.databaseId });
      
      // Find the property that is of type 'title'
      const titlePropEntry = Object.entries(response.properties).find(([_, prop]) => (prop as any).type === 'title');
      ...
      if (!backProp || (backProp as any).type !== 'rich_text') {
      ...
      if (this.deckId) {
        ...
        if (!deckProp || (deckProp as any).type !== 'relation') {
  ...
  private async getTemplatePageDetails(): Promise<{ icon: any; cover: any }> {
    if (!this.templateId) return { icon: null, cover: null };

    try {
      const response = await this.client.pages.retrieve({ page_id: this.templateId });
      return {
        icon: (response as any).icon,
        cover: (response as any).cover
      };
    } catch {
