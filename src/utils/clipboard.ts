...
export class ClipboardManager {
  ...
  private looksLikeBase64Image(content: string): boolean {
    // 检查是否看起来像base64编码的图片数据
    if (content.length < 100) return false; // 太短不可能是图片
    if (content.length > 10000000) return false; // 太长超过限制
  ...
  async watchClipboard(callback: (content: string, isImage: boolean) => void): Promise<() => void> {
    let lastContent = '';
  ...
  // 平台特定的剪贴板图片获取方法
  private async getMacClipboardImage(): Promise<string | null> {
    try {
  ...
  private async getWindowsClipboardImage(): Promise<string | null> {
    try {
      const tempDir = os.tmpdir();
      const tempImagePath = path.join(tempDir, `clipboard_${Date.now()}.png`);
  ...
  private async getLinuxClipboardImage(): Promise<string | null> {
    try {
      const tempDir = os.tmpdir();
      const tempImagePath = path.join(tempDir, `clipboard_${Date.now()}.png`);
